...
#[derive(StructOpt)]
#[structopt(name = "mdbx-tui", about = "TUI for MDBX storage engine")]
pub struct Opt {
...
static ENVIRONMENT: Lazy<(mdbx::Environment<NoWriteMap>, Vec<String>)> = Lazy::new(|| {
    let opt = Opt::from_args();

    let tables = read_max_dbs(&opt.env_path).unwrap();
    let mut b = mdbx::Environment::<NoWriteMap>::new();
    b.set_max_dbs(tables.len());
    b.set_flags(::mdbx::EnvironmentFlags {
        mode: ::mdbx::Mode::ReadOnly,
        ..Default::default()
    });
...
fn main() -> anyhow::Result<()> {
    ...
    siv.add_layer({
        ...
        select.set_on_submit(move |siv: &mut Cursive, table: &String| {
            siv.pop_layer();
            let db = tx.open_db(Some(table)).unwrap();
            siv.add_layer({
